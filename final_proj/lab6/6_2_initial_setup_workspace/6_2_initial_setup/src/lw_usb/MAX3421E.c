...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE write_byte[] = {reg + 2, val};

	int value = XSpi_Transfer(&SpiInstance, write_byte, NULL, 2);

	if (value != 0){
		xil_printf("There is an error in function: 'void MAXreg_wr' with value != 0\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE write_byte[nbytes + 1];
	write_byte[0] = reg + 2;

	for (int i = 0; i < nbytes; i++){
		write_byte[i + 1] = data[i];
	}

	int value = XSpi_Transfer(&SpiInstance, write_byte, NULL, nbytes+1);

	if (value != 0){
		xil_printf("There is an error in function: 'BYTE* MAXreg_wr' with value != 0\n");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int value = XSpi_Transfer(&SpiInstance, write_byte, read_byte, 2);

	if (value != 0){
		xil_printf("There is an error in function: 'BYTE* MAXreg_rd' with value != 0\n");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE read_byte[nbytes+1];

	int value = XSpi_Transfer(&SpiInstance, write_byte, read_byte, nbytes+1);

	if (value != 0){
		xil_printf("There is an error in function: 'BYTE* MAXbytes_rd' with value != 0\n");
	}

	for(int i=0; i<nbytes; i++){
		data[i] = read_byte[i+1];
	}
